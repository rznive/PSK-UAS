...
const updateCommentHandler = async (req, res) => {
  ...
  try {
    ...
    if (error) {
      return res.status(500).json({ message: "Failed to fetch the comment", error });
    }
    ...
    const { data, updateError } = await supabase
      ...
      .update({ comment_text })
      .eq('id', id)
      .select('id, activity_report_id, user_id, nama, comment_text')  // Ensure you select all necessary fields
      .single();  // Ensure it's fetching a single row of updated data

    if (updateError) {
      return res.status(500).json({ message: "Failed to update the comment", updateError });
    }
    ...
    res.status(200).json({
      ...
      data,  // Should contain the updated comment data
    });
  } catch (error) {
