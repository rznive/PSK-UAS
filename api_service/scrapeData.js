...
const fetchDetailPage = async (url) => {
  try {
    ...
    const visualObservation = $("h6.slim-card-title")
      .filter((_, element) => $(element).text().trim() === "Pengamatan Visual")
      .next("p")
      .text()
      .trim() || "Tidak ada data visual.";

    const klimatologiObservation = $("h6.slim-card-title")
      .filter((_, element) => $(element).text().trim() === "Klimatologi")
      .next("p")
      .text()
      .trim() || "Tidak ada data klimatologi.";

    const gempaObservation = $("h6.slim-card-title")
      .filter((_, element) => $(element).text().trim() === "Pengamatan Kegempaan")
      .next("p")
      .text()
      .trim() || "Tidak ada data gempa.";

    return {
      visualObservation,
      klimatologiObservation,
      gempaObservation,
    };
  } catch (error) {
    console.error(`Error fetching detail page ${url}:`, error.message);
    return {
      visualObservation: "Error fetching data.",
      klimatologiObservation: "Error fetching data.",
      gempaObservation: "Error fetching data.",
    };
  }
};
...
const scrapePage = async (pageNumber, endDate) => {
  try {
    ...
    if ($("title").text().trim() === "Halaman Tidak Ditemukan") {
      ...
      return await scrapePage(pageNumber, newEndDate);
    }

    const dataToInsert = [];
    const timelineItems = $(".timeline-item");
    for (const element of timelineItems) {
      const namaGunung = $(element).find(".timeline-title a").text().trim();
      if (!namaGunung) continue;
      ...
      const {
        ...
        klimatologiObservation,
        gempaObservation,
      } = await fetchDetailPage(detailUrl);
      ...
      dataToInsert.push({
        ...
        image_url: imageUrl,
        pengamatan_visual: visualObservation,
        klimatologi_visual: klimatologiObservation,
        gempa_visual: gempaObservation,
      });
    }
...
(async () => {
  try {
    ...
    while (hasMorePages) {
      const { dataToInsert, hasNextPage, endDate: updatedEndDate } = await scrapePage(
        currentPage,
        endDate
      );
